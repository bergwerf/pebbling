...
Ltac edge_ne u v :=
  match goal with
  | H : E u v |- _ => apply irrefl_ne in H as ?
  end.
...
Lemma list_choice {X Y} (P : X -> Y -> Prop) xs :
  Forall (λ x, ∃ y, P x y) xs -> ∃ ys, Forall2 P xs ys.
Proof.
induction xs; inversion_clear 1; [exists []; done|].
destruct H0 as [y ?]; apply IHxs in H1 as [ys ?].
exists (y :: ys); constructor; done.
Qed.

Lemma unnest_fallback {X} (P : X -> Prop) Q (xs : list X) :
  (∀ x, x ∈ xs -> P x ∨ Q) -> (∀ x, x ∈ xs -> P x) ∨ Q.
Proof.
induction xs as [|x xs]; intros H; [left; set_solver|].
assert (Hx : x ∈ x :: xs) by set_solver; apply H in Hx as []; [|right; done].
destruct IHxs; [set_solver|left; set_solver|right; done].
Qed.

Lemma NoDup_dec_witness `{dec : EqDecision X} (xs : list X) :
  NoDup xs + {∃ i j x, i ≠ j ∧ xs !! i = Some x ∧ xs !! j = Some x}.
Proof.
induction xs as [|x xs]; [left; constructor|]; destruct IHxs as [IH|IH].
- dec (x ∈ xs); [right|left; constructor; done].
  apply elem_of_list_lookup_1 in H as [i Hi]; exists 0, (S i), x; done.
- right; destruct IH as (i & j & x' & ? & ? & ?); exists (S i), (S j), x'; cbn.
  repeat split; [lia|done|done].
Qed.

Lemma Forall2_elem_of_l {X Y} (P : X -> Y -> Prop) xs ys x :
  Forall2 P xs ys -> x ∈ xs -> ∃ y, y ∈ ys ∧ P x y.
Proof.
intros HP Hx; apply elem_of_list_lookup_1 in Hx as [i Hi].
eapply Forall2_lookup_l in Hi as (y & Hy & Hxy); [exists y|done].
split; [apply elem_of_list_lookup_2 in Hy|]; done.
Qed.
...
{ unfold xs; intros x Hx; apply elem_of_list_filter in Hx as [Hx _].
  ...
  destruct (diameter2 x t) as [|[]]; [done|right|left; done].
  eexists; split; [move_pebble x t; done|simpl_alter; lia]. }
apply unnest_fallback in Hxs as [Hxs|]; [|done].
apply list.Forall_forall, list_choice in Hxs as [ys ys_spec].
...
{ intros y Hy; dec (c y = 0); [left; done|right].
  ...
  edge_ne x y; edge_ne y t; eexists; split.
...
{ eapply Forall2_lookup_r in Hi as (x1 & Hi & Hx); [|done]; destruct Hx;
  ...
  edge_ne x1 y; edge_ne x2 y; edge_ne y t.
...
apply elem_of_list_lookup_2 in Hy; clear i; edge_ne v y; edge_ne y t;
...
{ intros x ?; assert (Hx : x ∈ xs) by (rewrite Hxs; set_solver).
  ...
  destruct (diameter2 x v) as [|[|(z & ? & ?)]]; [done|right|].
  ...
  - dec (z = t); [right|left].
    + eexists; split; [edge_ne t v; move_pebble v t; done|simpl_alter; lia].
    + exists z; done. }
apply unnest_fallback in Hxs' as [Hxs'|]; [|done].
apply list.Forall_forall, list_choice in Hxs' as [zs zs_spec].
...
{ intros z Hz; destruct (decide (z ∈ ys)) as [Hys|Hys]; [right|left; done].
  ...
  edge_ne x z; edge_ne z v; eexists; split.
...
{ assert (Hz : z ∈ zs) by (apply elem_of_list_lookup_2 in Hi; done);
  ...
  edge_ne x1 z; edge_ne x2 z; edge_ne z v; eexists; split.
...
apply submseteq_length in length0; cbn in length0;
erewrite app_length, <-Forall2_length in length0; [|done].
erewrite <-Forall2_length with (k:=ys) in length0; [|done].
assert (length xs = length xs' + 1) by (rewrite Hxs; cbn; lia).
assert (vs0_length : length vs0 ≥ 2 * length xs) by lia.
revert weight2 H Ht ys_spec vs0_length; clear; intros.
(* Determine w with c w ≥ 4. *)
destruct (decide (Forall (λ k, k ≤ 3) (c <$> xs))) as [Hxs|Hxs].
apply summation_forall_le in Hxs; rewrite fmap_length in Hxs; lia.
apply not_Forall_Exists in Hxs; [|solve_decision].
apply list.Exists_exists in Hxs as (k & Hk & Hw); cbn in Hw.
apply elem_of_list_fmap in Hk as (w & -> & Hwxs).
eapply Forall2_elem_of_l in Hwxs as (y & _ & Hy); [|done]; destruct Hy.
edge_ne w y; edge_ne y t; eexists; split.
move_pebble w y; move_pebble w y; move_pebble y t; done.
simpl_alter; [|intro|intro]; subst; lia.
Qed.
